...
import com.jahid.productratings.book.BookAnalysis;
import com.jahid.productratings.book.BookAnalysisAdapter;
import com.jahid.productratings.product.Product;

import java.util.ArrayList;
import java.util.List;

public class BookAnalysisActivity extends AppCompatActivity {

    FirebaseDatabase database;
    DatabaseReference myRef;

    RecyclerView recyclerView;
    List<BookAnalysis> mobileList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        myRef = database.getReference("bookanalysis");
        ...
        myRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot brandSnapshot : dataSnapshot.getChildren()) {
                    ...
                    score = brandSnapshot.child("comment_rating").getValue(Integer.class);


                    Log.e(dataSnapshot.getKey(), dataSnapshot.getChildrenCount() + "");

                    BookAnalysis product4 = new BookAnalysis(title, author,
                            boibazar_noOfRated, boibazar_price, boibazarRating, boibazar_url,
                            imageUrl, rokomari_noOfrated, rokomari_price, rokomari_rating,rokomari_url,score);
                    //System.out.println("Title: " + words);
                    mobileList.add(product4);
                    //mobileList.add(product);

                }
                adapter = new BookAnalysisAdapter(getApplicationContext(), mobileList);
